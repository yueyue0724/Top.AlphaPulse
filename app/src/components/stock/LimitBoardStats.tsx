...
export function LimitBoardStats({
    lianbanStats,
    zhabanCount,
    fengbanRate,
    maxLianban,
    ...
    return (
        <div className={cn('p-4', className)}>
            {/* 标题 */}
            <div className="text-sm font-medium text-slate-500 mb-3">连板统计</div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
                ...
                <div className="flex flex-col items-center p-2 rounded-lg bg-amber-50">
                    ...
                    <span className="text-[10px] text-slate-500">封板率</span>
                </div>

                {/* 炸板数 */}
                <div className="flex flex-col items-center p-2 rounded-lg bg-slate-50">
                    <Zap className="w-4 h-4 text-slate-500 mb-1" />
                    <span className="text-lg font-bold text-slate-600 font-mono">{zhabanCount}</span>
                    <span className="text-[10px] text-slate-500">炸板</span>
                </div>
            ...
            <div className="space-y-1.5">
                {lianbanData.map((item) => (
                    <div key={item.label} className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono text-slate-700 w-6 text-right">{item.count}</span>
                    </div>
                ))}
            </div>
            ...
            {topIndustries.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-100">
                    <div className="text-xs text-slate-400 mb-2">涨停热门行业</div>
